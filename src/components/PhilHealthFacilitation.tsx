...
export default function PhilHealthFacilitation() {
  ...
  const fetchData = async (url = 'https://api-dbosca.phoenix.com.ph/api/philhealth-facilitation') => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      let finalUrl = url;
      if (url === 'https://api-dbosca.phoenix.com.ph/api/philhealth-facilitation') {
  ...
  const handleStatusChange = async (record: PhilHealthRecord, newStatus: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://api-dbosca.phoenix.com.ph/api/philhealth-facilitation/${record.id}`, {
        method: "PUT",
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
  ...
  const handleConfirmRejection = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://api-dbosca.phoenix.com.ph/api/philhealth-facilitation/${selectedRecord.id}`, {
        method: "PUT",
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
  ...
  const handleFileAction = async (path: string, filename: string, action: 'view' | 'download') => {
    try {
      ...
      const queryParams = `path=${encodeURIComponent(path)}&name=${encodeURIComponent(filename)}&action=${action}`;
  ...
  return (
    <div className="space-y-10">
      <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-4xl font-black tracking-tight text-[#0F172A]">PHILHEALTH FACILITATION</h2>
          <p className="text-slate-500 font-medium mt-1 text-lg tracking-tight">Health Insurance Membership Management</p>
        </div>
      </header>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
          <div className="flex flex-col sm:flex-row items-center gap-4 flex-1">
            ...
            <div className="relative flex-1 w-full sm:max-w-md">
              ...
              <input 
                type="text" 
                placeholder="Search by name, SCID, or PhilHealth ID..."
                value={searchTerm}
          ...
          <table className="w-full text-left border-collapse min-w-[1100px]">
            <thead>
              <tr className="bg-[#0F172A]">
                <th className="px-6 py-5 text-[11px] font-bold text-white uppercase tracking-widest">Date Submission</th>
                <th className="px-6 py-5 text-[11px] font-bold text-white uppercase tracking-widest">SCID Number</th>
                <th className="px-6 py-5 text-[11px] font-bold text-white uppercase tracking-widest">Full Name</th>
                <th className="px-6 py-5 text-[11px] font-bold text-white uppercase tracking-widest">PhilHealth ID Number</th>
                <th className="px-6 py-5 text-[11px] font-bold text-white uppercase tracking-widest text-center">Address</th>
                <th className="px-6 py-5 text-[11px] font-bold text-white uppercase tracking-widest text-center">Status</th>
                <th className="px-6 py-5 text-[11px] font-bold text-white uppercase tracking-widest text-center">Action</th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                ...
                filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-6">
                      <p className="text-xs font-black text-[#E11D48] tracking-widest">
                    ...
                    <td className="px-6 py-6 text-center">
                      <p className="text-xs font-bold text-slate-600 uppercase tracking-tight">
                        {item.address?.barangay}, {item.address?.city}, {item.address?.province}
                      </p>
                    </td>
                    <td className="px-6 py-6 text-center">
                      <div className={cn(
                        ...
                        <div className={cn(
                          ...
                          (item.application?.status === 'verified' || item.application?.status === 'Verified') ? "bg-emerald-500" : 
                          item.application?.status === 'rejected' ? "bg-rose-500" : "bg-amber-500 animate-pulse"
                        )} />
                        {item.application?.status}
                      </div>
                    </td>
                    <td className="px-6 py-6 overflow-visible relative">
                            ...
                            {activeDropdown === item.id && (
                              <>
                                ...
                                <motion.div
                                  ...
                                  className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 z-20 py-2 overflow-hidden"
                                >
