          ...
          <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Reference: ID-{record.id || 'N/A'}</p>
              </div>
            </div>
            <button 
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-8">
              <div className="space-y-1">
                ...
                <p className="text-sm font-bold text-slate-900">{formatDate(record.application_date)}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Modality</p>
                <p className="text-sm font-bold text-slate-900 uppercase">{record.modality}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</p>
                <span className={cn(
                  "px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest inline-block",
                  record.status?.issuance_status === 'approved' ? "bg-emerald-50 text-emerald-600" :
                  record.status?.issuance_status === 'pending' ? "bg-amber-50 text-amber-600" : 
                  record.status?.issuance_status === 'rejected' ? "bg-rose-50 text-rose-600" :
                  record.status?.issuance_status === 'printed' ? "bg-blue-50 text-blue-600" :
                  record.status?.issuance_status === 'issued' ? "bg-indigo-50 text-indigo-600" : "bg-slate-50 text-slate-600"
                )}>
                ...
                {['approved', 'disapproved'].includes(record.status?.issuance_status?.toLowerCase()) && record.date_reviewed && (
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                    REVIEWED: {new Date(record.date_reviewed).toLocaleDateString('en-US', { month: 'long', day: '2-digit', year: 'numeric' }).toUpperCase()}
                  </p>
                )}
          ...
          >
             ...
             <div className="flex-1 flex flex-col items-center p-8 overflow-y-auto bg-slate-50/30">
                <div 
                  ...
                  className="transition-transform duration-200 mt-10 mb-20"
                >
                    {viewSide === 'front' ? (
                        /* ID FRONT */
                        <div className="w-[450px] h-[280px] bg-white border border-slate-300 rounded-lg overflow-hidden flex flex-col font-sans text-black relative shadow-lg print:shadow-none print:border-slate-400">
                            {/* Header */}
                            <div className="flex flex-col items-center pt-2 relative">
                                <img 
                                  ...
                                  className="absolute left-6 top-3 w-12 h-12 object-contain" 
                                  alt="San Juan Seal"
                                />
                                <img 
                                  src="https://res.cloudinary.com/dx20khqe5/image/upload/v1777035949/Group_271_rxyfrp.png" 
                                  className="absolute right-6 top-3 w-14 h-12 object-contain" 
                                  alt="Secondary Logo"
                                />
                                <span className="text-[7px] font-black tracking-tighter uppercase leading-tight">Republic of the Philippines</span>
                                <span className="text-lg font-black uppercase leading-none tracking-tight">City of San Juan</span>
                                <span className="text-[7px] font-black uppercase leading-tight tracking-tighter">Office of the Senior Citizen Affairs (OSCA)</span>
                            </div>

                            {/* Red Name Bar */}
                            <div className="bg-[#ef4444] text-white pt-1 px-4 mt-2 h-14 flex flex-col items-center justify-center relative">
                                <span className="text-[7px] font-black uppercase leading-none mb-1 opacity-90">Full Name</span>
                                <h3 className="text-xl font-black uppercase truncate tracking-tight w-full text-center">{record.full_name}</h3>
                            </div>

                            {/* Content Container */}
                            <div className="px-4 py-1 flex flex-1 flex-col">
                                {/* Address Section - Centered */}
                                <div className="mt-2 text-center w-full relative">
                                    <p className="text-[10px] font-black uppercase truncate leading-none">
                                      {(record.user_details?.address || record.address || '') + (record.barangay ? `, BRGY. ${record.barangay}` : '')}
                                    </p>
                                    <p className="text-[10px] font-black uppercase truncate leading-tight">San Juan City</p>
                                    <div className="border-t border-slate-300 w-[60%] mx-auto mt-0.5"></div>
                                    ...
                                    <div className="flex-1 flex flex-col justify-between pr-4">
                                      <div className="flex justify-between mt-auto mb-2">
                                          <div className="w-[23%] text-center">
                                            <div className="text-[8px] font-black uppercase mb-0.5 h-3 flex items-end justify-center tracking-tighter">{record.user_details?.birth_date || record.birth_date || '---'}</div>
                                            <div className="border-t border-slate-300 mx-auto w-full"></div>
                                            <div className="text-[6px] font-black tracking-tighter uppercase mt-0.5">Birthday</div>
                                          </div>

                                          <div className="w-[10%] text-center">
                                            <div className="text-[8px] font-black mb-0.5 h-3 flex items-end justify-center">{record.user_details?.age || record.age || '---'}</div>
                                            <div className="border-t border-slate-300 mx-auto w-full"></div>
                                            <div className="text-[6px] font-black tracking-tighter uppercase mt-0.5">Age</div>
                                          </div>

                                          <div className="w-[12%] text-center">
                                            <div className="text-[8px] font-black uppercase mb-0.5 h-3 flex items-end justify-center">{record.user_details?.sex || record.sex || '---'}</div>
                                            <div className="border-t border-slate-300 mx-auto w-full"></div>
                                            <div className="text-[6px] font-black tracking-tighter uppercase mt-0.5">Gender</div>
                                          </div>

                                          <div className="w-[30%] text-center relative">
                                            <div className="h-6 flex items-end justify-center mb-0.5 px-2">
                                              {record.files?.signature_url || record.signature_url ? (
                                                  <img src={record.files?.signature_url || record.signature_url} className="h-5 object-contain" />
                                              ) : (
                                                  <div className="h-5" />
                                              )}
                                            </div>
                                      ...
                                      <div className="pb-1">
                                          ...
                                          <p className="text-[6px] font-black tracking-tighter uppercase leading-none">City Mayor</p>
                                      </div>
                                    </div>

                                    {/* Photo Side */}
                                    <div className="w-28 flex flex-col gap-1 items-end pt-1">
                                        <p className="text-[7px] font-black whitespace-nowrap tracking-tighter">PWD ID No.: <span className="font-bold">{record.scid_number || '---'}</span></p>
                                        <div className="w-24 h-28 border-2 border-black rounded-[1.5rem] overflow-hidden bg-slate-50 flex items-center justify-center relative translate-y-[-10px]">
                            ...
                            {/* Bottom Bars */}
                            <div className="bg-[#000080] h-2 w-full mt-auto" />
                            <div className="bg-[#ef4444] text-white py-1.5 text-center">
                                <p className="text-[11px] font-black uppercase tracking-[0.4em] leading-none">Senior Citizen ID Card</p>
                            </div>
                        </div>
                    ) : (
                        /* ID BACK */
                        <div className="w-[450px] h-[280px] bg-white border border-slate-300 rounded-lg overflow-hidden flex flex-col font-sans text-black relative shadow-lg print:shadow-none print:border-slate-400">
                            ...
                            <div className="bg-[#ef4444] text-white py-2 text-center">
                                <p className="text-xs font-black uppercase tracking-widest leading-none">SAN JUAN CITY</p>
                            </div>
                        </div>
                    )}
                </div>
...
export default function IdIssuanceModule({ type, applications }: IdIssuanceModuleProps) {
  ...
  const fetchIdIssuances = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      ...
      const response = await fetch("https://api-dbosca.phoenix.com.ph/api/id-issuances", { 
        method: "GET",
        headers 
      });
  ...
  const fetchMasterlist = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("https://api-dbosca.phoenix.com.ph/api/masterlist?per_page=100", { 
        method: "GET",
        headers 
      });
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://api-dbosca.phoenix.com.ph/api/id-issuances/${statusRecord.id}`, {
        method: "PATCH",
        headers,
      ...
      if (statusRecord.citizen_id) {
        try {
          ...
          await fetch(`https://api-dbosca.phoenix.com.ph/api/masterlist/${statusRecord.citizen_id}`, {
            method: "PATCH",
            headers,
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        {type === 'Management' ? (
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative">
              ...
              <input 
                ...
                className="pl-12 pr-6 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-[#0F172A] w-64 outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all"
              />
            </div>
            <div className="relative">
              ...
              <select 
                ...
                className="pl-12 pr-10 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-[#0F172A] outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all appearance-none cursor-pointer min-w-[140px]"
              >
            ...
            <div className="relative">
              ...
              <select 
                ...
                className="pl-12 pr-10 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-[#0F172A] outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all appearance-none cursor-pointer min-w-[160px]"
              >
            ...
            <button className="flex items-center gap-2 px-6 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg shadow-slate-200">
              <Download className="w-4 h-4" />
              Export
            </button>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-2 p-1.5 bg-slate-100 rounded-[1.5rem] border border-slate-200 shadow-inner">
              {['New ID', 'Renewal', 'Replacement'].map((tab) => (
                <button
                  ...
                  className={cn(
                    ...
                    walkInTab === tab 
                      ? "bg-white text-rose-500 shadow-md shadow-slate-200/50" 
            ...
            {walkInTab === 'New ID' && (
              <button 
                ...
                className="flex items-center gap-2 px-6 py-3.5 bg-rose-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-600 transition-all shadow-lg shadow-rose-200"
              >
            ...
            <div className="relative flex-1 min-w-[300px]">
              ...
              <input 
                ...
                className="w-full pl-12 pr-6 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-[#0F172A] outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all shadow-sm"
              />
            </div>
          </div>
        )}
          ...
          <div className="overflow-x-auto rounded-[2.5rem]">
            {type === 'Management' ? (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    ...
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Birthdate</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Barangay</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Application Date</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Request Type</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Modality</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">ID Status</th>
                      ...
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-6 py-6">
                          <div className="flex items-center gap-2 text-slate-600">
                            ...
                            <span className="text-xs font-bold">{formatDate(item.birth_date)}</span>
                          </div>
                        </td>
                        <td className="px-6 py-6">
                          <div className="flex items-center gap-2 text-slate-600">
                            <MapPin className="w-3.5 h-3.5 text-slate-400" />
                            <span className="text-xs font-bold uppercase">{item.barangay}</span>
                          </div>
                        </td>
                        <td className="px-6 py-6">
                          <span className="text-xs font-bold text-slate-600">{formatDate(item.application_date)}</span>
                        </td>
                        <td className="px-6 py-6">
                        ...
                        <td className="px-6 py-6">
                          <span className="text-xs font-bold text-slate-600 uppercase">{item.modality}</span>
                        </td>
                        <td className="px-6 py-6">
                          <div className="flex flex-col gap-1.5">
                            <span className={cn(
                              "px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm w-fit",
                              item.status?.issuance_status === 'approved' ? "bg-emerald-500 text-white" :
                              item.status?.issuance_status === 'pending' ? "bg-amber-500 text-white" : 
                              (item.status?.issuance_status === 'rejected' || item.status?.issuance_status === 'disapproved') ? "bg-rose-500 text-white" :
                              item.status?.issuance_status === 'released' ? "bg-indigo-500 text-white" : 
                              item.status?.issuance_status === 'printed' ? "bg-blue-500 text-white" :
                              item.status?.issuance_status === 'issued' ? "bg-indigo-500 text-white" : "bg-slate-400 text-white"
                            )}>
                            ...
                            {['approved', 'disapproved'].includes(item.status?.issuance_status?.toLowerCase()) && item.date_reviewed && (
                              <div className="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap">
                                <span>REVIEWED:</span>
                                <span>{new Date(item.date_reviewed).toLocaleDateString('en-US', { month: 'long', day: '2-digit', year: 'numeric' }).toUpperCase()}</span>
                              </div>
                            )}
                                    ...
                                    >
                                      <div className="p-2 space-y-1">
                                        <button
                                          ...
                                          className="w-full flex items-center gap-3 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl transition-all"
                                        >
                                          <Eye className="w-4 h-4" />
                                          View Details
                                        </button>
                                        ...
                                        {item.status?.issuance_status?.toLowerCase() === 'pending' && (
                                          <button
                                            ...
                                            className="w-full flex items-center gap-3 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-[#2563EB] hover:bg-blue-50 hover:text-blue-700 rounded-xl transition-all"
                                          >
                                        ...
                                        {['approved', 'released'].includes(item.status?.issuance_status?.toLowerCase()) && (
                                          <button
                                            onClick={() => handlePreviewId(item)}
                                            className="w-full flex items-center gap-3 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-rose-600 hover:bg-rose-50 hover:text-rose-700 rounded-xl transition-all"
                                          >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    {['Replacement', 'Renewal'].includes(walkInTab) && (
                      <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">SCID Number</th>
                    )}
                    <th className={cn("py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest", ['Replacement', 'Renewal'].includes(walkInTab) ? "px-6" : "px-8")}>Fullname</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Birthdate</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Barangay</th>
                    {walkInTab === 'Renewal' ? (
                      <>
                        <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Released Date</th>
                        <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Expiration Date</th>
                      </>
                    ) : (
                      <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Address</th>
                    )}
                    <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {filteredMasterlist.length > 0 ? (
                    filteredMasterlist.map((item) => (
                      <tr key={item.citizen_id || item.id} className="hover:bg-slate-50/50 transition-colors group">
                        {['Replacement', 'Renewal'].includes(walkInTab) && (
                          <td className="px-8 py-6">
                        ...
                        <td className={cn("py-6", ['Replacement', 'Renewal'].includes(walkInTab) ? "px-6" : "px-8")}>
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center group-hover:bg-white transition-colors">
                              <User className="w-5 h-5 text-slate-400" />
                            </div>
                        ...
                        <td className="px-8 py-6 text-right">
                          <button 
                            ...
                            className="px-6 py-2.5 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg shadow-slate-200/50"
                          >
                            Proceed with ID
                          </button>
                        </td>
      ...
      <WalkInIdIssuanceModal 
        ...
        onSuccess={() => {
          if (type === 'Management') fetchIdIssuances();
          else fetchMasterlist();
        }}
      />
...
const IdEditProfileModal = ({ 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://api-dbosca.phoenix.com.ph/api/id-issuances/${record.id}`, {
        method: "POST", 
        headers: {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    ...
                    <div className="relative border-2 border-dashed border-slate-200 rounded-2xl p-6 hover:border-blue-500 transition-all text-center">
                      <input type="file" onChange={(e) => handleFileUpload(e, 'req1_url')} className="absolute inset-0 opacity-0 cursor-pointer" />
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative border-2 border-dashed border-slate-200 rounded-2xl p-6 hover:border-blue-500 transition-all text-center">
                      <input type="file" onChange={(e) => handleFileUpload(e, 'req2_url')} className="absolute inset-0 opacity-0 cursor-pointer" />
              ...
              <div className="space-y-6 text-center">
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  <button className="relative flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-black text-[10px] uppercase tracking-widest">
                    ...
                    <input type="file" onChange={(e) => handleFileUpload(e, 'photo_url')} className="absolute inset-0 opacity-0 cursor-pointer" />
                  </button>
                </div>
              </div>
            )}
            ...
            {activeTab === 'Signature' && (
              <div className="space-y-6">
                ...
                <div className="flex justify-center gap-4">
                  ...
                  <button className="relative flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-black text-[10px] uppercase tracking-widest">
                    <Upload className="w-4 h-4" /> Upload
                    <input type="file" onChange={(e) => handleFileUpload(e, 'signature_url')} className="absolute inset-0 opacity-0 cursor-pointer" />
                  </button>
                </div>
              </div>
            )}
